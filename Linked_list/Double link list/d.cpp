...
void traversal(struct header *head)
{
    ...
    while (wptr != NULL)
    {
        printf("\n Address of current node- %p, Roll- %d, Sex- %c, Address of next node- %p, Address of prev node- %p",
               wptr, wptr->roll, wptr->sex, wptr->next, wptr->prev);
        wptr = wptr->next;
    }
...
void insert_after(struct header *head, struct student *temp, int position)
{
    ...
    while (counter < position && wptr != NULL)
    { // By this loop wptr will go and stand at a second last point of a specified position...
...
{
    if (head->first == NULL)
    { // check list is empty or not ...if it is empty it means underflow
        printf("\n Underflow!!!!");
        getch();
        return;
    }
